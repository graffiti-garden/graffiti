    ...
    try {
        switch (data.action) {
            ...
            case RequestAction.Encrypt:
                ...
                const theirPublicKey = theirPublicKeyString ? base64Decode(theirPublicKeyString) : undefined
                const ciphertext = await actorManager.encryptPrivateMessage(
                    base64Decode(plaintextString), theirPublicKey, nonce
                )
            ...
            case RequestAction.Decrypt:
                ...
                const theirPublicKeyy = theirPublicKeyStringg ? base64Decode(theirPublicKeyStringg) : undefined
                const plaintext = await actorManager.decryptPrivateMessage(
                    base64Decode(ciphertextString), theirPublicKeyy, nonce
                )
...
<template>
    ...
    <main>
        ...
        <template v-else-if="uiState == UIState.Managing || !chosen">
            <fieldset>
                ...
                <ul>
                    <li v-for="[uri, nickname] of Object.entries(actors)" :key="uri">
                        <label :for="uri">
                            <input type="radio" :id="uri" :value="uri" v-model="selected" @click="() => {
                            ...
                            <form v-if="editing == uri" @submit.prevent="rename(uri)">
                                <input type="text" v-model="editingNickname" v-focus
                            ...
                            <div :class="menuOpen == uri ? ['dropdown', 'open'] : 'dropdown'">
                                ...
                                <menu v-if="menuOpen == uri" v-click-away="() => menuOpen = null">
                                    <li>
            ...
            <form>
                <button v-if="selected" @click.prevent="actorManager.chooseActor(selected)" class='highlight'
                    ref="loginButton">
