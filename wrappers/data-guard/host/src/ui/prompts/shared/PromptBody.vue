...
<style scoped>
.summary { position: relative; width: 100%; border: 1px solid var(--border-color); border-radius: 0.5rem; padding: 0.75rem 1.1rem 1rem; background: color-mix(in srgb, var(--background-color-interactive) 62%, var(--background-color)); transition: border-color 120ms ease, background-color 120ms ease; }
.summary.expandable { cursor: pointer; }
.summary.expandable:hover, .summary.expandable:focus-visible { border-color: var(--border-color-hover); background: color-mix(in srgb, var(--background-color-interactive) 78%, var(--background-color)); outline: none; }
.summary-line { line-height: 1.45; }
.summary-line > strong { margin-right: 0.35rem; }
details { border: 0; background: transparent; font-size: 1rem; }
summary { position: absolute; left: 50%; bottom: 0; display: inline-flex; transform: translate(-50%, 50%); border: 1px solid var(--border-color); border-radius: 999px; padding: 0.15rem 0.65rem; background: var(--background-color-interactive); list-style: none; cursor: pointer; }
